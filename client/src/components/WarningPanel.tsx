  ...
  return (
    <div className="bg-gray-900 rounded-md p-2 mb-1">
      ...
      {activeWarnings.length > 0 && (
        <div className="mt-1 overflow-hidden rounded bg-gray-800 cursor-pointer" onClick={() => dismissWarning(activeWarnings[0].id)}>
          <div className="flex items-center p-1 text-xs">
            <span className={`material-icons text-sm mr-1 ${getSeverityColor(activeWarnings[0].severity)}`}>
              error
            </span>
            <span className="font-medium">{activeWarnings[0].title}:</span>
            <span className="ml-1 text-gray-300 truncate">{activeWarnings[0].description}</span>
          </div>
        </div>
      )}
    </div>
